...
public class MySwaggerResourcesProvider implements SwaggerResourcesProvider {
    ...
    public List<SwaggerResource> get() {
        ...
        routeLocator.getRoutes().stream().filter(route -> route.getId() != null)
                .filter(route -> !self.equals(route.getId()))
                .forEach(route -> routeHosts.add(route.getId()));

        // 记录已经添加过的server
        Set<String> dealed = new HashSet<>();
        routeHosts.forEach(instance -> {
            // 拼接url,gateway大写,zuul小写
            //String url = "/" + instance.toUpperCase() + SWAGGER2URL;
